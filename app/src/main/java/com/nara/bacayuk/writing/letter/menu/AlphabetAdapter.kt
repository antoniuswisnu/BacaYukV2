...
class AlphabetAdapter(val listener: AdapterListener) :
    RecyclerView.Adapter<AlphabetAdapter.RecentAdapterViewHolder>() {
    ...
    override fun onBindViewHolder(holder: RecentAdapterViewHolder, position: Int) {
        holder.view.apply {
            ...
            binding.txtAbjad.text = data?.tulisHuruf
            binding.imgChecklist.invisible()

            if(data?.reportTulisHuruf != null){
                if(data.reportTulisHuruf.materiTulisHurufKapital && data.reportTulisHuruf.latihanTulisHurufKapital && data.reportTulisHuruf.materiTulisHurufNonKapital && data.reportTulisHuruf.latihanTulisHurufNonKapital){
